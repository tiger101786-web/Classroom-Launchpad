...
(async () => {
  try {
    ...
    assert.equal(removedReplacement.response.status, 200);
    ...
    const thirdGradeSpotlight = await request("/api/student-spotlights", {
      method: "POST",
      headers: { "Content-Type": "application/json", Origin: origin, Cookie: teacherCookie },
      body: JSON.stringify({
        studentEmail: "",
    ...
    assert.match(appSource, /spotlight-board-top/);
    assert.match(appSource, /spotlight-board-accent/);
    assert.match(appSource, /spotlight-board-stand/);
    ...
    assert.match(appSource, /Assignment folder preview/);
