...
assert.match(app, /finishedMrsLevandoskeCelebrationVideos\.has\(video\)[\s\S]*?video\.ended \|\| \(duration > 0 && video\.currentTime >= duration - 0\.08\)[\s\S]*?finishedMrsLevandoskeCelebrationVideos\.add\(video\)/, "A completed Mrs. Levandoske celebration should hold its final frame instead of restarting or changing during the same level.");
assert.match(app, /shouldHoldFinishedMrsLevandoskeCelebration[\s\S]*?finishedMrsLevandoskeCelebrationVideos\.has\(video\)[\s\S]*?video\.paused && !shouldHoldFinishedMrsLevandoskeCelebration/, "Forced media synchronization should not restart a finished Mrs. Levandoske celebration.");
