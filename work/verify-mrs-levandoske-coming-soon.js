...
assert.match(app, /const COLT_RUN_SHARE_PATH = "\/colt-run";/, "Colt Run should have a stable share path.");
assert.match(app, /let screen = \{ name: isDirectColtRunPath\(\) \? "coltRun" : "home" \};/, "The share path should open directly into Colt Run.");
assert.match(app, /data-colt-run="copyLink">Copy Game Link</, "Colt Run should provide a visible copy-link control.");
assert.match(app, /navigator\.clipboard\.writeText\(shareUrl\)/, "The copy-link control should copy the direct game URL.");
assert.match(server, /pathname === "\/colt-run\/"[\s\S]*?Location: "\/colt-run"/, "The trailing-slash game path should redirect to the canonical share URL.");
assert.match(server, /pathname === "\/" \|\| pathname === "\/colt-run"[\s\S]*?"\/index\.html"/, "The server should serve the Launchpad game app at the direct Colt Run path.");
