...
async function run() {
  ...
  try {
    ...
    await page.getByRole("button", { name: "Name Your Colt", exact: true }).click();
    await page.getByRole("button", { name: "Glasses On", exact: true }).click();
    await page.waitForFunction(() => window.__savedColtCustomization?.glassesVisible === true);
    const sunglasses = page.locator(".launchpad-colt-sunglasses");
    await page.waitForFunction(() => !document.querySelector(".launchpad-colt-sunglasses")?.hidden);
    assert.equal(await sunglasses.getAttribute("src"), "assets/launchpad-colt-sunglasses.png?v=20260905-idle-prototype1");
    assert.equal(await page.getByRole("button", { name: "Glasses Off", exact: true }).isVisible(), true);
    await page.getByRole("button", { name: "Close Colt naming panel", exact: true }).click();
    await page.locator(".launchpad-colt-character").click();
    ...
    assert.equal(await sunglasses.isHidden(), true, "The idle prototype glasses remained visible over an untracked animation.");
