...
async function run() {
  ...
  try {
    ...
    assert(Math.abs((controlsBounds.width / controlsBounds.height) - (1672 / 1032)) < 0.08, "The Colt controls artwork is distorted.");
    ...
    const bathControl = controlsPanel.locator('[data-colt-control="bath"]');
    const bathControlBounds = await bathControl.boundingBox();
    ...
    assert.equal(await bathControl.evaluate(element => getComputedStyle(element).position), "static", "Give Bath must occupy a regular matching artwork slot.");
    assert.equal(await bathControl.evaluate(element => getComputedStyle(element).borderTopWidth), "0px", "Give Bath must use the same border treatment as the other panel options.");
