...
(async()=>{
 ...
 try{
  ...
  for(const scene of ['highland','squishy']){
  await page.emulateMedia({reducedMotion:'no-preference'});
  await page.setContent('<style>'+fs.readFileSync('launchpad-scenes.css','utf8')+'</style><div class="launch-scene scene-'+scene+'" style="width:300px;height:300px"><span class="launch-scene-particles">'+Array.from({length:9},(_,n)=>`<i style="--n:${n}"></i>`).join('')+'</span></div>');
  ...
  console.log(scene+' motion: steady linear speed, staggered loops, pause and reduced-motion checks passed.');
  }
