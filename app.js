...
function startColtRunGame() {
  ...
  if (!isGitHubPagesHost) sceneTwoVideo.src = "assets/colt-run-bg-02.mp4?v=20260707-scene2-replace";
  ...
  if (!isGitHubPagesHost) sceneFourVideo.src = "assets/colt-run-bg-04.mp4?v=20260708-scene4-replace";
  ...
  const getTransparentLavaRockVideoFrame = () => {
    ...
    const tailTopFeather = lavaRockVideoFrameSize * 0.2;
    const tailRightFeather = lavaRockVideoFrameSize * 0.16;
    for (let pixelIndex = 0; pixelIndex < lavaRockVideoFrameSize * lavaRockVideoFrameSize; pixelIndex += 1) {
      const alphaIndex = pixelIndex * 4 + 3;
      ...
      const edgeFade = Math.min(topFade, rightFade);
  ...
  const setKey = (name, value) => {
    if (name === "left" || name === "right" || name === "jump") keys[name] = value;
  };
  const keyMap = {
    ArrowLeft: "left",
    KeyA: "left",
